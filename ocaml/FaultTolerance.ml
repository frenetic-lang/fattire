...
let rec action_choice_to_choice lst = match lst with
  | [a] -> a
  | a :: lst -> Choice (a, action_choice_to_choice lst)
...
let strip_tag  = Mod (Header SDN_Types.Vlan, VInt.Int16 0xFFFF)

let stamp_path_tag pathTag tag = 
  (* If we set the VLAN, the controller will push a new tag on. Should probably fix that *)
  Seq (Mod (Header SDN_Types.Vlan, VInt.Int16 pathTag),
       Mod (Header SDN_Types.VlanPcp, VInt.Int16 tag))
...
let match_tag pathTag tag = 
  (And (Test (Header SDN_Types.Vlan, pathTag),
        Test (Header SDN_Types.VlanPcp, tag)))
...
let next_port_from_k_tree sw topo pathTag tree = 
  ...
  | (tag, KLeaf_t host) -> 
    ...
    Seq(strip_tag, Mod(Header SDN_Types.InPort, VInt.Int32 p1))
  | (tag, KTree_t (sw', _)) -> 
    let p1,p2 = G.get_ports topo sw sw' in
    Seq(stamp_tag tag, Mod(Header SDN_Types.InPort, VInt.Int32 p1))
  | (tag, KRoot_t (h,sw)) ->
...
let rec policy_from_k_tree' inport tree topo path_tag tag = 
  ...
  match tree with
    ...
    | KTree_t(sw', children) -> 
      ...
      let backup = lpar(And( Test(Switch, VInt.Int64 sw), And (match_tag path_tag tag, Test (Header SDN_Types.InPort, inport))), 
			children_actions) in
      let next_hops = List.map (next_hop_from_k_tree sw' topo) children in
      let children_pols = List.fold_left 
	(fun a (sw'', inport,tree) -> 
	  Par(a, policy_from_k_tree' (VInt.Int32 inport) (snd tree) topo path_tag (VInt.Int16 (fst tree)))) trivial_pol next_hops in
...
let next_port_from_k_tree_root sw topo pathTag tree = 
  ...
  | (tag, KTree_t (sw', _)) ->
    let p1,p2 = G.get_ports topo sw sw' in
    Seq(stamp_path_tag pathTag tag, Mod(Header SDN_Types.InPort, VInt.Int32 p1))
...
let policy_from_k_tree pr tree topo path_tag tag =  
  ...
  match tree with
    | KRoot_t(G.Host h, KTree_t(G.Switch sw, children)) -> 
      ...
      let backup = lpar(And( Test (Switch, VInt.Int64 sw), And (pr, Test (Header SDN_Types.InPort, VInt.Int32 inport))),
			children_ports) in
      let next_hops = List.map (next_hop_from_k_tree (G.Switch sw) topo) children in
      let children_pols = List.fold_left 
	(fun a (sw'', inport,treeTag) -> 
	  Par(a, policy_from_k_tree' (VInt.Int32 inport) (snd treeTag) topo (VInt.Int16 path_tag) (VInt.Int16 (fst treeTag)))) trivial_pol next_hops in
      Par(backup, children_pols)
